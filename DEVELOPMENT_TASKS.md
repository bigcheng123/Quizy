...
Quizy 是一款面向小学生的"开机即锁屏答题"软件。计算机开机后自动以全屏锁定方式启动，孩子必须完成当日规定数量的语文与数学题目，才能解锁桌面进入正常使用。家长（管理员）可通过隐藏入口 + 密码进入后台，配置年级、题量、密码并管理题库与查看答题记录。
...
7. **管理员触发区 `#admin-trigger`**：右下角约 60×60px 透明区域，`cursor:default`，无视觉痕迹。
...
- [ ] 5 次快速点击右下角隐藏区域 → 出现密码弹窗 → 输入默认密码 `123456` → 进入管理后台。
